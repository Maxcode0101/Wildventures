...
class StripePaymentTest(TestCase):
    def setUp(self):
        ...
        self.booking = Booking.objects.create(
            ...
            + timedelta(days=5),  # Booking starts 5 days from today
            end_date=date.today()
            + timedelta(days=10),  # Booking ends 10 days from today
    ...
    def test_create_checkout_session(self, mock_stripe_session_create):
        ...
        mock_stripe_session_create.return_value = type(
            "obj",
            (object,),
            {"url": "http://stripe.test/checkout", "id": "cs_test_dummy"},
        )
    ...
    @patch("booking.views.stripe.checkout.Session.create")
    def test_create_checkout_session_block_for_past_booking(
        ...
        self.assertTrue(
            any(
                "ongoing or in the past" in str(message).lower()
                for message in messages
            )
        )

    @patch("booking.views.stripe.checkout.Session.create")
    def test_create_checkout_session_block_for_ongoing_booking(
        self, mock_stripe_session_create
    ):
        """
