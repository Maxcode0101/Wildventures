...
def contact(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                "The message has been sent. We'll get back at you soon!",
            )
...
def campervan_list(request):
    ...
    brands_list = list(
        Campervan.objects.exclude(brand__isnull=True)
        .values_list("brand", flat=True)
        .distinct()
        .order_by("brand")
    )
    models_list = list(
        Campervan.objects.exclude(model__isnull=True)
    ...
    for brand in (
        ...
        brand_models = list(
            Campervan.objects.filter(brand__iexact=brand)
    ...
    for model in (
        ...
        model_brands = list(
            Campervan.objects.filter(model__iexact=model)
...
def check_availability(request):
    ...
    try:
        ...
        return JsonResponse(
            {"error": "An unexcepted error occured"}, status=400
        )
